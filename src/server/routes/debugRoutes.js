...
export function registerDebugRoutes(app, context) {
    const { state } = context;

    app.get('/debug-ui', async (req, res) => {
        if (!state.cdpConnection) return res.status(503).json({ error: 'CDP not connected' });
    ...
    try {
        ...
        try {
            // 1. Get Frames
            const { frameTree } = await state.cdpConnection.call("Page.getFrameTree");
            ...
            // 2. Map Contexts
            const contexts = state.cdpConnection.contexts.map(c => ({
                id: c.id,
                name: c.name,
                origin: c.origin,
            ...
            for (const ctx of contexts) {
                try {
                    const result = await state.cdpConnection.call("Runtime.evaluate", {
                        expression: EXP,
                        returnByValue: true,
                        contextId: ctx.id
                    });
